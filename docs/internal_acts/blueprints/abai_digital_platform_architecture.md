# Архитектурный паспорт: Интегрированная автоматизированная система «[V_SIS_SYSTEM_NAME]»
...
> Утверждено Ректором-Председателем Правления [V_LEGAL_FORM] «[V_UNIVERSITY_FULL_NAME]» Тлеп Б. А. для прохождения испытаний и аттестации в **АО «Государственная техническая служба» (ГТС)** в соответствии с ЕТИКТ и ИБ (ПП РК № 832) и Приказом МЦРИАП РК № 153/НҚ.

---

## 1. Назначение и концепция платформы

**Интегрированная автоматизированная система «[V_SIS_SYSTEM_NAME]»** (условное обозначение — **[V_SIS_SYSTEM_NAME]**) является ключевой корпоративной студенческой информационной системой (Student Information System, SIS) и средой управления академическим процессом Университета. 
...
### Ключевые цели создания [V_SIS_SYSTEM_NAME]:
1. **Сквозная цифровизация жизненного цикла обучающегося:** от подачи онлайн-заявления абитуриента в модуле **[V_ADMISSION_MODULE]** до выдачи диплома государственного образца с QR-верификацией.
...
Комплекс технической документации [V_SIS_SYSTEM_NAME] включает 22 детально регламентированных модуля с описанием бизнес-логики, экранных форм, API-эндпоинтов и структур баз данных:

```mermaid
graph TD
    subgraph "Абитуриент и Прием"
        REG[Модуль [V_ADMISSION_MODULE]<br/>Онлайн-подача и прием]
    end

    subgraph "Ядро SIS — [V_SIS_SYSTEM_NAME]"
...
#### 1. Модуль «[V_ADMISSION_MODULE]» (Приемная кампания)
...
- **Архитектурная связь с SIS:** Единая точка входа (SSO) через корпоративную учетную запись; расписание и контингент групп синхронизируются из [V_SIS_SYSTEM_NAME], а текущие и рубежные баллы (Р1, Р2) транслируются обратно в сводную экзаменационную ведомость [V_SIS_SYSTEM_NAME].
...
## 3. Матрица связей платформы [V_SIS_SYSTEM_NAME] с внутренними нормативными актами (ВНД)

| Модуль [V_SIS_SYSTEM_NAME] | Регулирующий внутренний акт Университета | Ответственное подразделение / должность |
| :--- | :--- | :--- |
...
| **[V_ADMISSION_MODULE]** | Положение о Приемной комиссии, Правила приема | Ответственный секретарь Приемной комиссии |
...
1. Исходный код и архитектурная документация платформы [V_SIS_SYSTEM_NAME] передаются на статический и динамический анализ защищенности (SAST/DAST).
